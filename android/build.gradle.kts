...
android {
	...
	defaultConfig {
		...
		// AzePSP: versionCode en az 1 olmalidir
            val safeVersionCode = if (gitVersionCode > 0) gitVersionCode else 1
            val safeVersionName = if (gitVersionName != "v0.0.0" && gitVersionName.isNotEmpty()) gitVersionName else "1.0.0-AzePSP"
            if (true) {
			println("INFO: AzePSP Version Name=$safeVersionName Code=$safeVersionCode")
		...
		file("versionname.txt").writeText(safeVersionName)
		file("versioncode.txt").writeText(safeVersionCode.toString())
