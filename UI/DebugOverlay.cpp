...
void DrawFPS(UIContext *ctx, const Bounds &bounds) {
	...
	// AzePSP: versiya yazisi
	const char *azepspLabel = "AzePSP v1.0 - AI Enhanced";
	ctx->Draw()->DrawText(ubuntu24, std::string_view(azepspLabel), bounds.x2() - offset, bounds.y + 34, 0xc0000000, ALIGN_TOPRIGHT | FLAG_DYNAMIC_ASCII);
	ctx->Draw()->DrawText(ubuntu24, std::string_view(azepspLabel), bounds.x2() - offset - 1, bounds.y + 33, 0xFFBC9200, ALIGN_TOPRIGHT | FLAG_DYNAMIC_ASCII);
	ctx->Draw()->SetFontScale(1.0f, 1.0f);
	ctx->Flush();
	ctx->RebindTexture();
}
