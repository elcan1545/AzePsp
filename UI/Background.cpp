...
// AzePSP: Azərbaycan bayrağı rəngli dalğa animasiyası
// Mavi (#0092BC), Qırmızı (#EF3340), Yaşıl (#509E2F)
class WaveAnimation : public Animation {
public:
        void Draw(UIContext &dc, double t, float alpha, Lin::Vec3 focus) override {
                // Azərbaycan bayrağının üç rəngi — AABBGGRR formatında
                const uint32_t colorBlue  = colorAlpha(0xFFBC9200, alpha * 0.22f); // Göy   #0092BC
                const uint32_t colorRed   = colorAlpha(0xFF4033EF, alpha * 0.20f); // Qırmızı #EF3340
                const uint32_t colorGreen = colorAlpha(0xFF2F9E50, alpha * 0.18f); // Yaşıl  #509E2F
                ...
                // Hər zolaq ekranın üçdə birini əhatə edir
                float thirdH = bounds.h / 3.0f;
                ...
                for (int n = 0; n < steps; n++) {
                        ...
                        // --- Göy zolaq (yuxarı üçdə bir) ---
                        float waveBlue = sin(i * 0.005 + t * 0.80) * 0.04f
                                       + sin(i * 0.002 + t * 0.25) * 0.02f
                                       + sin(i * 0.001 + t * 0.30) * 0.02f;
                        float blueTop    = waveBlue * thirdH;
                        float blueBottom = thirdH;
                        if (blueTop < blueBottom) {
                                dc.Draw()->RectVGradient(x, blueTop, nextX, blueBottom, colorBlue, 0x00000000);
                                dc.Draw()->RectVGradient(x, blueTop - 2.0f * g_display.pixel_in_dps_y, nextX, blueTop, 0x00000000, colorBlue);
                        }

                        // --- Qırmızı zolaq (orta üçdə bir) ---
                        float waveRed = sin(i * 0.0044 + t * 0.60) * 0.04f
                                      + sin(i * 0.003  + t * 0.15) * 0.02f
                                      + sin(i * 0.002  + t * 0.40) * 0.01f;
                        float redTop    = thirdH + waveRed * thirdH;
                        float redBottom = thirdH * 2.0f;
                        if (redTop < redBottom) {
                                dc.Draw()->RectVGradient(x, redTop, nextX, redBottom, colorRed, 0x00000000);
                                dc.Draw()->RectVGradient(x, redTop - 2.0f * g_display.pixel_in_dps_y, nextX, redTop, 0x00000000, colorRed);
                        }

                        // --- Yaşıl zolaq (aşağı üçdə bir) ---
                        float waveGreen = sin(i * 0.006 + t * 0.45) * 0.04f
                                        + sin(i * 0.002 + t * 0.20) * 0.02f
                                        + sin(i * 0.001 + t * 0.35) * 0.02f;
                        float greenTop    = thirdH * 2.0f + waveGreen * thirdH;
                        float greenBottom = bounds.h;
                        if (greenTop < greenBottom) {
                                dc.Draw()->RectVGradient(x, greenTop, nextX, greenBottom, colorGreen, 0x00000000);
                                dc.Draw()->RectVGradient(x, greenTop - 2.0f * g_display.pixel_in_dps_y, nextX, greenTop, 0x00000000, colorGreen);
                        }
                }
